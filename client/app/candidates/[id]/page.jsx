    ...
    try {
      if (status === "shortlisted") {
        const { data } = await axios.post(
          `https://newsaless-2.onrender.com/api/candidate/sendemail/${params.id}`
        );
        if (data.status === 200) {
    ...
    const fetchData = async () => {
      ...
      try {
        const { data } = await axios.get(
          `https://newsaless-2.onrender.com/api/candidate/${params.id}`,
          {
      ...
      {isLoading ? (
        ...
        <div className="min-h-screen bg-slate-200">
          <div className="p-4 bg-slate-50">
            <p className="font-bold text-lg sm:text-xl uppercase text-center my-2">
              Candidate profile
            </p>
          </div>
          ...
          {candidate ? (
            candidate.map((item) => (
              <div
                key={item._id}
                className="bg-slate-200 my-4 h-full sm:p-4 mx-4"
              >
                ...
                <div className="p-1 sm:p-4 md:p-4 bg-slate-300 rounded-md">
                  <p className="font-semibold">
                    Name:
                    <span className="mx-2 text-base font-normal text-black">
                      {item.firstName} {item.lastName}
                    </span>
                  </p>
                  <p className="font-semibold">
                    Status:
                    <span className={`mx-2 text-base font-semibold text-black`}>
                      {item.status}
                    </span>
                  </p>
                  <p className="font-semibold">
                    Admin:
                    <span className={`mx-2 text-base font-medium text-black`}>
                      {item.adminAction}
                    </span>
                  </p>
                  <p className="font-semibold">
