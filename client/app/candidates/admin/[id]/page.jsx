...
const Candidates = () => {
    ...
    const handelPostRequest = async () => {
        ...
        setStatusMessage('')
        const endpoint =
            modalData === 'selected' ? `shortlist/${params.id}` :
                modalData === 'discarded' ? `discard/${params.id}` :
                    modalData === 'pending' ? `pending/${params.id}` :
                        modalData === 'invited' ? `invited/${params.id}` :
                            modalData === 'employee' ? `employee/${params.id}` :
                                '';

        try {
            const response = await axios.post(
                `http://localhost:3000/api/candidate/${endpoint}`,
    ...
    return <>
        <div className=" min-h-screen w-full bg-slate-100 p-4 ">
            <p className=" p-2 sm:p-3 md:p-4  bg-white text-black font-semibold text-lg sm:text-xl md:text-3xl shadow-xl rounded-md">Candidates Portal</p>
