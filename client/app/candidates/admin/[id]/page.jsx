...
const Candidates = () => {
  ...
  const currentItems = candidate.slice(
  ...
  const table = (
    <div className="relative overflow-x-auto shadow-md sm:rounded-lg bg-white sm:p-2 ">
      <table className="w-full text-sm text-left rtl:text-right bg-slate-900 text-white  ">
        ...
        <tbody>
          {currentItems.map((item, i) => (
            <tr
              key={item._id}
              className="bg-slate-800 text-slate-400 p-2 text-center hover:bg-slate-900 hover:text-white cursor-pointer border-b-2 border-slate-600"
            >
              <td>{i + 1}</td>
              <td>
              ...
              <td>{item.state}</td>
              <td>
                {item.degree} {item.branch} {item.passingYear}
              </td>
              <td>{item.status}</td>
              <td className="pt-2">
                <textarea
                  className="resize-none p-2 text-slate-200 bg-slate-700 rounded-sm focus:outline-none cursor-pointer"
                  name=""
                  id=""
                  cols="25"
                  rows="4"
              ...
              <td>
                ...
                >
                  <button className="py-1 px-3 mx-2 bg-blue-600 text-slate-50">
                    View
                  </button>
                </Link>
              </td>
  ...
  const handelPostRequest = async () => {
    ...
    try {
      const response = await axios.post(
        `https://newsaless-2.onrender.com/api/candidate/${endpoint}`,
        { candidateId } // This sends candidateId in the body of your POST request
      );
  ...
  const handelSendMail = async () => {
    try {
      const data = await axios.post(
        "https://newsaless-2.onrender.com/api/candidate/sendemail"
      );
  ...
  const handelSendSingleMail = async () => {
    try {
      const response = await axios.post(
        `https://newsaless-2.onrender.com/api/candidate/sendemail/${userId}`
      );
    ...
    const fetchAllCandidates = async () => {
      try {
        const { data } = await axios.get(
          `https://newsaless-2.onrender.com/api/candidate/admin-candidate/${params.id}`,
          {
  ...
  useEffect(() => {
    const filteredCandidates =
      statusFilter === "all"
        ? allCandidate
        : allCandidate.filter((item) => item.status === statusFilter);
  ...
  return (
    <>
      <div className=" min-h-screen w-full bg-slate-100 p-4 ">
        <p className=" p-2 sm:p-3 md:p-4  bg-white text-black font-semibold text-lg sm:text-xl md:text-3xl shadow-xl rounded-md">
          Candidates Portal
        </p>
        <div className="mt-6   flex flex-wrap sm:flex-row  justify-between items-center  gap-2">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className="px-2 md:px-4 py-1 md:py-2 bg-green-600 text-white sm:rounded-2xl"
            >
              All
            </button>
            ...
            <button
              ...
              className="px-2 md:px-4 py-1 md:py-2 bg-gray-600 text-white sm:rounded-2xl"
            >
              Pending
            </button>
            ...
            <button
              onClick={() =>
                ` ${handleStatusFilter("shortlisted")}, ${setMail(true)}`
              }
              className="px-2 md:px-4 py-1 md:py-2 bg-blue-600 text-white sm:rounded-2xl"
            >
              Shortlisted
            </button>
            ...
            <button
              onClick={() =>
                `${handleStatusFilter("discarded")}, ${setMail(false)}`
              }
              className="px-2 md:px-4 py-1 md:py-2 bg-red-600 text-white sm:rounded-2xl"
            >
              Discarded
            </button>
            ...
            <button
              onClick={() =>
                `${handleStatusFilter("invited")} , ${setMail(false)}`
              }
              className="px-2 md:px-4 py-1 md:py-2 bg-green-600 text-white sm:rounded-2xl"
            >
              Invited
            ...
            <button
              onClick={() => handleStatusFilter("employee")}
              className="px-2 md:px-4 py-1 md:py-2 bg-gray-600 text-white sm:rounded-2xl"
            >
              employee
            </button>
        ...
        <div className="text-center mt-10 mb-2 shadow-gray-300">
          <button
            ...
            }  text-slate-50 px-2 py-1 rounded-md mx-3`}
            onClick={handlePreviouBtn}
          ...
          <button
            className={`${
              currentPage === totalPages ? "bg-blue-400" : "bg-blue-600 "
            } text-slate-50 px-2 py-1 rounded-md mx-3`}
        ...
        <div class="modal-dialog">
          <div class="modal-content">
              ...
              <div className="d-flex flex-wrap gap-2 align-items-center justify-content-center ">
                <button
                  ...
                  className={`" text-white p-3 " ${
                    modalData === "pending" ? "bg-gray-600" : "bg-gray-300"
                  }`}
                >
                  Pending
                </button>
                <button
                  ...
                  className={`" text-white p-3 " ${
                    modalData === "discarded" ? "bg-red-600" : "bg-red-300"
                  }`}
                >
                  Discarded
                </button>
                ...
                <button
                  ...
                  className={`" text-white p-3 " ${
                    modalData === "selected" ? "bg-blue-600" : "bg-blue-300"
                  }`}
                >
                  Shortlisted
                </button>
                <button
                  ...
                  className={`" text-white p-3 " ${
                    modalData === "invited" ? "bg-gray-600" : "bg-gray-300"
                  }`}
                >
                  Invited
                </button>
                <button
                  ...
                  className={`" text-white p-3 " ${
                    modalData === "employee" ? "bg-green-600" : "bg-green-300"
                  }`}
                >
                  Employee
                </button>
            ...
            <div class="modal-footer">
              <button
                type="button"
                className="bg-gray-600 text-white px-2 md:px-6 py-1 md:py-3"
                data-bs-dismiss="modal"
              >
                Close
              </button>
              <button
                ...
                type="button"
                className="bg-blue-600 text-white px-2 md:px-6 py-1 md:py-3"
                data-bs-dismiss="modal"
              >
                Save changes
              </button>
