...
const Page = ({ params }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/api/employee/employee/total-customers/${params.id}`);
  ...
  if (isFetching) {
    return <div className='min-h-screen flex justify-center items-center font-semibold text-lg sm:text-xl md:text-2xl'>LOADING...</div>
  } else if (isError) {
    return <div className='min-h-screen flex justify-center items-center font-semibold text-lg sm:text-xl md:text-2xl'>Invalid Request Please Login</div>;
  } else if (!data) {
    return <div className='min-h-screen flex justify-center items-center font-semibold text-lg sm:text-xl md:text-2xl'>No data available.</div>;
  }
            ...
            <div className='h-[18rem] w-full flex flex-col justify-center  py-2  md:py-5 px-2 md:px-5  text-sm  md:text-base shadow-xl bg-slate-50  text-black rounded-xl gap-6'>

              {
                edata && <div className='flex flex-col gap-2' key={edata.id}>
                  ...
                  <div>
                    <div className=''>
                      <span className='md:text-lg font-semibold mx-2'>Last Month Sales:</span>
                      {/* <span className='md:text-lg'>₹{edata.sale}</span> */}
                      <span className='md:text-lg'>₹{edata.lastMonthSale}</span>
                    </div>
                  </div>
                  <div>
                    <div className=''>
                      <span className='md:text-lg font-semibold mx-2'>Total Sales:</span>
                      {/* <span className='md:text-lg'>₹{edata.sale}</span> */}
            ...
            <div className='col-span-12  py-6 md:py-4  '>
              {custError ? <p className='text-center font-bold p-6 bg-white rounded-lg m-2'>{custError}</p> : <div className="bg-white shadow-md rounded p-4">
                ...
                <div className='text-center p-2 my-2'>
                  <button
                    className={`${currentPage === 1 ? "bg-blue-400" : "bg-blue-600"} text-slate-50  px-2 `}
                    onClick={handlePreviousBtn} disabled={currentPage === 1}>
                    Previous
                  </button>
                  <span className='font-medium mx-2'>Page {currentPage}/{totalPages}</span>
                  <button
                    className={`${currentPage >= totalPages ? "bg-blue-400" : "bg-blue-600"} text-slate-50  px-2 `}
                ...
                <div className='text-end'>
                  <form onSubmit={handleGoToPage} style={{ display: 'inline' }}>
                    <input
                      type="number"
