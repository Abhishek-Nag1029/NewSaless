...
const EditEmail = ({ params, socket }) => {
  ...
  const submitNewEmail = async (newEmail) => {
    try {
      const updateResponse = await updateAdminEmail({
        id: params.id,
        newEmail,
      }).unwrap();
    ...
    try {
      if (status === undefined) {
        const response = await axios.post(
          `https://newsaless-2.onrender.com/api/admin/checkpass/${params.id}`,
          emailData
        );
        setStatus(200);
        ...
        const response = await axios.post(
          `https://newsaless-2.onrender.com/api/admin/otp/${params.id}`,
          emailData
        );
        console.log("OTP matched successfully");
        setStatus(201);
      } else if (status === 201) {
        // const response = await axios.put(`https://newsaless-2.onrender.com/api/admin/updateEmail/${params.id}`, emailData)
        ...
        <form action="" onSubmit={handelSubmit}>
          <div className=" p-3">
            {status ? (
              ...
              <div className="my-2">
                <label className="md: text-lg" htmlFor="currentPassword">
                  Enter Current Password<span className="text-red-600">*</span>
                </label>
                <input
            ...
            {status === 200 ? (
              <div className="my-2">
                <label className="md: text-lg" htmlFor="OTP">
            ...
            {status === 201 ? (
              <div className="my-2">
                <label className="md: text-lg" htmlFor="newEmail">
                  Enter newEmail<span className="text-red-600">*</span>
                </label>
                <input
                  name="newEmail"
