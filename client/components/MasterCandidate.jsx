...
const MasterCandidate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await axios.put(
      `https://newsaless-2.onrender.com/api/master-admin/candidate/update/${editId}`,
      formData
    );
    ...
    const fetchCandidates = async () => {
      try {
        const response = await axios.get(
          "https://newsaless-2.onrender.com/api/master-admin/candidate-data",
          {
    ...
    <>
      <div className="overflow-x-auto my-2 p-3 shadow-lg h-full">
        ...
        <table className="min-w-full bg-white">
          ...
          <tbody>
            {currentCandidates.map((candidate, index) => (
              <tr
                className=" hover:bg-slate-100 hover:text-black  cursor-pointer"
                key={index}
              >
                ...
                <td className="border px-8 py-4 text-center">
                  <button
                    ...
                    className="mx-2 px-6 py-1 bg-blue-600 text-white"
                  >
                    Edit
                  </button>
                </td>
        ...
        <div className="flex justify-center items-center my-4">
          <button
            onClick={prevPage}
            className={`px-3 py-1 mx-1 bg-blue-600 text-white rounded ${
