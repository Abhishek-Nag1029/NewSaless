...
const EmployeesList = () => {
  ...
  useEffect(() => {
    ...
    const fetchEmployees = async () => {
      ...
      const sortField =
        key === "id" ? "_id" : key === "totalSale" ? "totalSale" : key; // Assume 'id' refers to '_id' in MongoDB
      try {
        const response = await axios.get(
          `https://newsaless-2.onrender.com/api/employee/fetchemployees`,
          {
            params: {
              page: currentPage,
              limit: 10,
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(
          `https://newsaless-2.onrender.com/api/customer/`
        );
    ...
    const fetchData = async () => {
      try {
        const { data } = await axios.get(
          `https://newsaless-2.onrender.com/api/employee/totalemp`,
          {
  ...
  const totalSalesAmount = empData.reduce(
  ...
  const totalSaleAmount = employees.reduce(
    (sale, employee) => sale + employee.totalSale,
    0
  );
  const totalCustomers = employees.reduce(
    (total, employee) => total + employee.totalCustomers,
    0
  );
              ...
              {employees &&
                employees.map((employee) => (
                  <tr key={employee.id}>
                    ...
                    <td className="border px-4 py-2">
                      {employee.totalCustomers}
                    </td>
                    <td className="border px-4 py-2">
                      ₹{employee.lastMonthSale * 0.1}
                    </td>
                    <td className="border px-4 py-2">
                      <Link
                        href={{
                      ...
                      >
                        <button className="py-2 px-5 rounded-lg bg-blue-600 text-slate-50">
                          View
                        </button>
                      </Link>
                    </td>
