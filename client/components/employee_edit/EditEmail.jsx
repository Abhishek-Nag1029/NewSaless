...
const EditEmail = ({ params, socket }) => {
  ...
  const submitNewEmail = async (newEmail) => {
    try {
      const updateResponse = await updateEmployeeEmail({
        id: params.id,
        newEmail,
      }).unwrap();
      setStatus(undefined);
    ...
    try {
      if (status === undefined) {
        const response = await axios.post(
          `https://newsaless-2.onrender.com/api/employee/checkPass/${params.id}`,
          emailData
        );
        setStatus(200);
        ...
        const response = await axios.post(
          `https://newsaless-2.onrender.com/api/employee/otp/${params.id}`,
          emailData
        );
