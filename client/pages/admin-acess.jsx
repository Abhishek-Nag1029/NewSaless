...
const admin_access = () => {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            const result = await axios.post('http://localhost:3000/api/master-admin/access', data);
            if (result.status === 200) {
                const token = sessionStorage.setItem("masterToken", "Master Admin Token granted")
                router.push('/master_admin')
            }
        } catch (error) {
                ...
                <form onSubmit={handleLogin} className='p-10 bg-slate-50 border-2'>
                    <div className='flex flex-col gap-2 justify-center p-2'>
                        ...
                        <input
                            ...
                            type="email" className='p-2 border-2' id="adminEmail" placeholder="Enter Your Email" required />

                    </div>
                    <div className='flex flex-col gap-2 justify-center p-2'>
                        <label htmlFor="adminAccess" className='font-medium'>Password</label>
                        <div className='w-full border-2'>
                            <input
                                onChange={e => setInpData({...inpData, password: e.target.value })}
                                type={`${toggle ? "text" : "password"}`} className='p-2 outline-none' id="adminAccess" placeholder="Enter Your Password" required />
                            <span onClick={e => setToggle(!toggle)} className='font-semibold p-2 cursor-pointer'> +</span>
                        </div>
